    ...
    <div className="bg-gray-800/50 rounded-lg shadow-2xl p-6 border border-gray-700 space-y-6 h-full flex flex-col">
      <div>
        ...
        <textarea
          ...
          className="w-full h-32 p-3 bg-gray-900 border border-gray-600 rounded-md focus:ring-2 focus:ring-teal-500 focus:outline-none transition"
          disabled={isLoading}
        />
        <div className="mt-4">
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*,.musicxml,.xml" disabled={isLoading} />
            <button onClick={handleFileSelectClick} className="w-full control-button bg-gray-700 hover:bg-gray-600 text-sm">
            ...
            {selectedFile && (
                <div className="text-xs text-center mt-2 text-gray-400 flex items-center justify-center">
                    ...
                    <button onClick={clearFile} className="ml-2 text-red-400 hover:text-red-300">&times;</button>
                </div>
            )}
        </div>
        <button onClick={handleImportClick} disabled={isLoading} className="mt-4 w-full control-button bg-green-700 hover:bg-green-600 text-lg font-bold">

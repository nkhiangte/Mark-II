...
export class SoundEngine {
  ...
  private activeSources: (OscillatorNode | GainNode)[] = [];
  ...
  private getNoteDuration(duration: keyof typeof DURATION_MAP, tempo: number): number {
  ...
  play(music: ParsedMusic, onEnded: () => void) {
    ...
    measures.flat().forEach(measure => {
      measure.notes.forEach(note => {
        ...
        totalDuration = Math.max(totalDuration, currentTime - (this.audioContext?.currentTime ?? 0));
      });
    });
  ...
  private scheduleNote(frequency: number, startTime: number, duration: number) {
    if (!this.audioContext) return;
